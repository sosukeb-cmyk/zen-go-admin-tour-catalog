...
export default function VehiclePricingTable({
  ...
  const handleLoad = () => {
    ...
    if (!pricing) {
      onChange({ office: officeLocation, rows: freshRows });
      return;
    }
    const prevByKey = new Map(pricing.rows.map((r) => [rowKey(r), r]));
  ...
  const overrideCount = pricing?.rows.filter((r) => r.custom).length ?? 0;
  const missingCount =
        ...
        <>
          {stale && (
            <div className="mt-3.5 flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              ...
              Loaded for the {pricing.office} office — this template is now
              set to {officeLocation}. Reload to refresh the list.
            </div>
          )}
          ...
          <div className="mt-3.5 overflow-hidden rounded-[10px] border border-gray-200">
            ...
            <div className="flex items-center justify-between gap-3 border-t border-gray-200 bg-gray-50 px-3 py-[9px]">
              <p className="text-xs text-gray-500">
                {pricing.rows.length} available vehicles from Pricing CMS ·{" "}
